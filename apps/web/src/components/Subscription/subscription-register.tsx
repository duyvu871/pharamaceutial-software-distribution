...
type SubscriptionAdminRegisterProps<Schema extends PlanType> = {
	onSelectPlan?: (plan: Schema) => void;
	activePlan?: Schema | null;
	type: "admin" | "branch";
}

export default function SubscriptionRegister<Schema extends PlanType, CreationSchema extends Partial<PlanType>>
({type, onSelectPlan, activePlan}: SubscriptionAdminRegisterProps<Schema>)
{
	...
	const resetFilter = () => {
		...
		getSubscriptionPlan(
			...
			.then((model) => {
				...
				setData(model.data as unknown as Schema[]);
				console.log(model);
			})
			.catch((error) => {
				showErrorToast(error.message);
