...
import { UserForm } from '@component/Form/user-form.tsx';
...
type CreationSchema = Partial<UserSchema>;
...
export default function UserDashboard() {
	...
	const updateOrCreate = useCallback(async (model: CreationSchema) => {
		try {
			...
			openActionOverlay();
			if (model.id) {
				const create = await createOrUpdateUserSlave(model);
				if (create) {
					setData((prev) => prev.map((item) => (item.id === create.id ? create : item)));
	...
	const FilterComponent = () => {
		...
		const applySearch = () => {
			...
			filter.editMultipleFilter({
				createdAt: `[${startDate?.getTime()}][${endDate?.getTime()}]`
			});
		};
	...
	const tableData: TableRender<Schema> = [
		...
		},
		{
			...
			render: (data) => {
				return (
					<Badge color={data.is_active ? "var(--main-color)" : "red"}>
						{data.is_active ? "Hoạt động" : "Khóa"}
					</Badge>
				);
			},
		},
	...
	return (
		...
		>
			...
			<Modal
				...
				title={
					<Typography size={"h5"} weight={"semibold"}>
						{activeDetail ? "Cập nhật thông tin đại lý" : "Thêm mới đại lý"}
					</Typography>
				}
				size="70vw"
				maw={"70vw"}
